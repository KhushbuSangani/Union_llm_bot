            ...
            for i in records["response"][1:]:
                for j in i:
                    ...
                    if a not in responses:
                        ...
                        responses[a][0]["buttons"].append(
                            {"title": j[0], "payload": j[1]}
                        )
        ...
        if records["response"][0] == "video":
            for i in records["response"][1:]:
                for j in i:
                    ...
                    if a not in responses:
                        responses[a] = [
                            {"attachment": {"type": "video", "payload": {"src": j}}}
                        ]
                    else:
                        responses[a][0]["attachment"].extend(
                            {"type": "video", "payload": {"src": j}}
                        )
        ...
        if records["response"][0] == "pdf":
            for i in records["response"][1:]:
                for j in i:
                    ...
                    if a not in responses:
                        responses[a] = [
                            {"custom": {"payload": "pdf_attachment", "url": j}}
                        ]
                    else:
                        responses[a][0]["custom"].extend(
                            {"payload": "pdf_attachment", "url": j}
                        )
